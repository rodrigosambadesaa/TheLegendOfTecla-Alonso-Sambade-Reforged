...
public class ComandoTirar implements Comando {
  ...
  public ComandoTirar(String comando, Juego juego, boolean bSoloJugador) throws ComandoExcepcion {
    ...
    try {
      for (Personaje personaje : this.juego.getMapa().getPersonajes()) {
        if (!bSoloJugador || personaje instanceof Jugador) {
          personaje
              .getMochila()
              .tirarObjeto(this.juego.getMapa().getCelda(personaje.getPosicionMapa()), comando);
        }
      }
    } catch (Exception ex) {
